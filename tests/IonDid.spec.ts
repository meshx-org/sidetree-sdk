...
describe("IonDid", async () => {
    ...
    describe("createLongFormDid()", async () => {
        ...
        it("vector test - should create a long-form DID correctly with Ed25519 keys.", async () => {
            ...
            const expectedMethodSpecificId =
                "did:ion:EiBks49Ah-ZxE1-6Se-PUDk_4_ffylpeqmfMCWUBwwTM-g:"
            expect(longFormDid).toEqual(expectedMethodSpecificId)
        })
        ...
        it("should throw error if given DID Document JWK is an array.", async () => {
            const [recoveryKey] = await IonKey.generateEs256kOperationKeyPair()
            const updateKey = recoveryKey
            const [anyDidDocumentKey] = await IonKey.generateEs256kDidDocumentKeyPair({
                id: "anyId",
        ...
        it("should throw error if given DID Document keys with the same ID.", async () => {
            const [recoveryKey] = await IonKey.generateEs256kOperationKeyPair()
            const updateKey = recoveryKey
            const [anyDidDocumentKey1] = await IonKey.generateEs256kDidDocumentKeyPair({
                id: "anyId",
                purposes: [IonPublicKeyPurpose.AssertionMethod],
            })
            const [anyDidDocumentKey2] = await IonKey.generateEs256kDidDocumentKeyPair({
                id: "anyId",
                purposes: [IonPublicKeyPurpose.Authentication],
            }) // Key ID duplicate.
        ...
        it("should throw error if resulting delta property exceeds maximum size.", async () => {
            ...
            const [anyDidDocumentKey3] = await IonKey.generateEs256kDidDocumentKeyPair({
                id: "anyId3",
                purposes: [IonPublicKeyPurpose.Authentication],
            })
            const [anyDidDocumentKey4] = await IonKey.generateEs256kDidDocumentKeyPair({
                id: "anyId4",
                purposes: [IonPublicKeyPurpose.Authentication],
            })
